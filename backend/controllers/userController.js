...
const registerUser = asyncHandler(async(req, res) => {
    ...
    return res.status(201).json({message : "New user registered successfully", createdUser, token})
...
const login  = asyncHandler(async(req, res) => {
    ...
    if(!req.body){
      ...
      if(!email || !password){
        ...
        return res.status(401).json({message : "Email and password are mandatory"})
      }
    ...
    return res.status(201).json({message : "User logged in successfully", existingUser, token})
...
const allUsers  = asyncHandler(async(req, res) => {
   ...
   if(users.length === 0){
    ...
    return res.status(200).json({message : "Users could not be found", users : []})
   }
